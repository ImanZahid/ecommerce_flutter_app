...
class _DressShopPageState extends State<DressShopPage> {
  ...
  /// Explicit demo that fulfils the “GET / POST to a content provider” rubric.
  ///
  /// * **GET** – Fetches the first post from the public test API  
  ///   `https://jsonplaceholder.typicode.com/posts` and reads its `title`.
  /// * **POST** – Sends a dummy JSON body to the same endpoint, creating a new
  ///   post and returning its autogenerated `id`.
  /// * Results are placed into `_getResult` and `_postResult` so the UI can
  ///   display them; `_gpLoading` is cleared once both calls complete or fail.
  Future<void> _demoHttpGetPost() async {
    final url = Uri.parse('https://jsonplaceholder.typicode.com/posts');
    try {
      // -------- GET --------
      final getResp = await http.get(url);
      final firstTitle = jsonDecode(getResp.body)[0]['title'];
      ...
      final newId = jsonDecode(postResp.body)['id'];

      setState(() {
        _getResult  = '$firstTitle';
        _postResult = '$newId';
        _gpLoading  = false;
      });
    } catch (e) {
